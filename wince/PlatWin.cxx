...
static void SetLogFont(LOGFONT &lf, const char *faceName, int characterSet, int size, bool bold, bool italic) {
	...
	int tlen = UCS2FromUTF8(faceName, strlen(faceName), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
	tbuf[tlen] = L'\0';
...
#ifdef UNDER_CE
	wchar_t tbuf[MAX_US_LEN];
	int tlen = UCS2FromUTF8(faceName_, strlen(faceName_), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
	tbuf[tlen] = L'\0';
#endif
	return
...
SurfaceImpl::SurfaceImpl() :
	...
	brush(0), brushOld(0),
	font(0), 	fontOld(0),
	bitmap(0), bitmapOld(0), paletteOld(0) 
	{
...
#else
		wchar_t tbuf[MAX_US_LEN];
		int tlen = UCS2FromUTF8(s, strlen(s), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
		tbuf[tlen] = L'\0';
		if (!::GetTextExtentExPoint(hdc, tbuf, Platform::Minimum(len, maxLenText),
			maxWidthMeasure, &fit, positions, &sz)) {
#endif
...
#else
	TCHAR tch = ch;
	::GetTextExtentPoint32(hdc, &tch, 1, &sz);
#endif
	return sz.cx;
}
...
void Window::SetTitle(const char *s) {
	...
	int tlen = UCS2FromUTF8(s, strlen(s), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
	tbuf[tlen] = L'\0';
	::SetWindowText(reinterpret_cast<HWND>(id), tbuf);
#endif
}
...
#else
	HINSTANCE hinstanceParent = hinstPlatformRes;
#endif
	id = ::CreateWindowEx(
...
#else
	return ::RegisterClass(&wndclassc) != 0;
#endif
}
...
#ifdef UNDER_CE
		wchar_t tbuf[MAX_US_LEN];
		int tlen = UCS2FromUTF8(modulePath, strlen(modulePath), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
		tbuf[tlen] = L'\0';
		h = ::LoadLibrary(tbuf);
#else
...
#ifdef UNDER_CE
			wchar_t tbuf[MAX_US_LEN];
			int tlen = UCS2FromUTF8(name, strlen(name), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
			tbuf[tlen] = L'\0';
			return static_cast<Function>(
					(void *)(::GetProcAddress(h, tbuf)));
#else
			return static_cast<Function>(
...
#ifdef UNDER_CE
		wchar_t tbuf[MAX_US_LEN];
		int tlen = UCS2FromUTF8(buffer, strlen(buffer), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
		tbuf[tlen] = L'\0';
		int idButton = ::MessageBox(0, tbuf, TEXT("Assertion failure"),
			MB_ABORTRETRYIGNORE|MB_ICONHAND|MB_SETFOREGROUND);
#else
		int idButton = ::MessageBox(0, buffer, TEXT("Assertion failure"),
