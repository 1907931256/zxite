...
#define PointToLONG(pt)		((long)(pt.x)+(((long)(pt.y))<<16))
#endif
...
sptr_t ScintillaWin::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SC_WIN_IDLE:
		...
		if (idler.state) {
			if (lParam || (WAIT_TIMEOUT==
#ifndef UNDER_CE
				MsgWaitForMultipleObjects(0,0,0,0, QS_INPUT|QS_HOTKEY))) {
#else
	...
	case WM_LBUTTONDOWN:
		{
#ifdef UNDER_CE
		Point point = Point::FromLong(lParam);
		SHRGINFO shrgi = {0};
		shrgi.cbSize        = sizeof(SHRGINFO);
		shrgi.hwndClient    = MainHWND();
		shrgi.ptDown.x      = point.x;
		shrgi.ptDown.y      = point.y;
		shrgi.dwFlags       = SHRG_RETURNCMD;
		if(GN_CONTEXTMENU == ::SHRecognizeGesture(&shrgi)) {
			POINT spt = {point.x, point.y};
			::ClientToScreen(MainHWND(), &spt);
			return WndProc(WM_CONTEXTMENU, 0, PointToLONG(spt));
		} else {
#endif
		...
		ButtonDown(Point::FromLong(lParam), 
			::GetMessageTime(),
			(wParam & MK_SHIFT) != 0,
			(wParam & MK_CONTROL) != 0,
			Platform::IsKeyDown(VK_MENU));
...
#else
	::ScrollWindowEx(MainHWND(), 0,
		vs.lineHeight * linesToMove, NULL, NULL, 0, NULL, SW_SCROLLCHILDREN);
#endif
...
void ScintillaWin::AddToPopUp(const char *label, int cmd, bool enabled) {
	...
	int tlen = UCS2FromUTF8(label, strlen(label), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
	tbuf[tlen] = L'\0';
#endif
	if (!label[0])
		::AppendMenu(hmenuPopup, MF_SEPARATOR, 0, TEXT(""));
	else if (enabled)
#ifdef UNDER_CE
		::AppendMenu(hmenuPopup, MF_STRING, cmd, tbuf);
	else
		::AppendMenu(hmenuPopup, MF_STRING | MF_GRAYED, cmd, tbuf);
#else
...
STDMETHODIMP DataObject_QueryGetData(DataObject *pd, FORMATETC *pFE) {
	...
	bool formatOK = (pFE->cfFormat == CF_TEXT) ||
		((pFE->cfFormat == CF_UNICODETEXT) && pd->sci->IsUnicodeMode())	|| (pFE->cfFormat == CF_HDROP)
#else
	bool formatOK = (pFE->cfFormat == CF_UNICODETEXT);
#endif
		;
	if (!formatOK ||
	    pFE->ptd != 0 ||
...
void ScintillaWin::ImeStartComposition() {
	...
	if (caret.active) {
		...
		if (stylesValid) {
			...
			{
				wchar_t tbuf[MAX_US_LEN];
				int tlen = UCS2FromUTF8(vs.styles[styleHere].fontName, strlen(vs.styles[styleHere].fontName), tbuf, sizeof(tbuf)/sizeof(wchar_t)-1);
				tbuf[tlen] = L'\0';
				_tcscpy(lf.lfFaceName, tbuf);
			}
#endif
...
STDMETHODIMP ScintillaWin::GetData(FORMATETC *pFEIn, STGMEDIUM *pSTM) {
	...
	bool formatOK = (pFEIn->cfFormat == CF_TEXT) ||
		((pFEIn->cfFormat == CF_UNICODETEXT) && IsUnicodeMode()) || (pFEIn->cfFormat == CF_HDROP);
#else
	bool formatOK = (pFEIn->cfFormat == CF_UNICODETEXT);
#endif
	if (!formatOK ||
	    pFEIn->ptd != 0 ||
...
#else
		result = ::RegisterClass(&wndclassc) != 0;
#endif
	}
...
#ifndef STATIC_BUILD
#ifdef UNDER_CE
extern "C" int APIENTRY DllMain(HANDLE hInstance, DWORD dwReason, LPVOID) {
#else
