...
FilePath SciTEGTK::GetDefaultDirectory() {
	const char *where = getenv("ZXITE_HOME");
#ifdef SYSCONF_PATH
	if (!where) {
		where = SYSCONF_PATH;
	}
...
int SciTEGTK::WindowMessageBox(GUI::Window &w, const GUI::gui_string &msg, int style) {
	if (!messageBoxDialog) {
		...
		if (style & MB_ABOUTBOX) {
			...
			SetAboutMessage(scExplanation, "Zxite");
		} else {
...
void SciTEGTK::AboutDialog() {
	WindowMessageBox(wSciTE, GUI::gui_string("Zxite\nby Neil Hodgson neilh@scintilla.org ."),
	                 MB_OK | MB_ABOUTBOX);
}
...
void SciTEGTK::CreateMenu() {
	...
	SciTEItemFactoryEntry menuItemsHelp[] = {
	                                          ...
	                                          {"/Help/_Zxite Help", "", menuSig, IDM_HELP_SCITE, 0},
	                                          {"/Help/_About Zxite", "", menuSig, IDM_ABOUT, 0},
	                                      };
...
bool SciTEGTK::CheckForRunningInstance(int argc, char *argv[]) {
	...
	GPatternSpec *pattern = g_pattern_spec_new("Zxite.*.in");
