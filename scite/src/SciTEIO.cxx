...
#ifdef unix
const GUI::gui_char propUserFileName[] = GUI_TEXT(".ZxiteUser.properties");
#else
// Windows
const GUI::gui_char propUserFileName[] = GUI_TEXT("ZxiteUser.properties");
#endif
const GUI::gui_char propGlobalFileName[] = GUI_TEXT("ZxiteGlobal.properties");
			...
			if (CurrentBuffer()->isDirty || props.GetInt("are.you.sure.on.reload") != 0) {
				if ((0 == dialogsOnScreen) && (newModTime != CurrentBuffer()->fileModLastAsk)) {
					...
					if (CurrentBuffer()->isDirty) {
						...
						msg = LocaliseMessage(
						          "The file '^0' has been modified outside Zxite. Should it be reloaded?",
	...
	if (!filePath.IsUntitled()) {
		...
		if (props.GetInt("save.deletes.first")) {
			...
			if ((newModTime != 0) && (CurrentBuffer()->fileModTime != 0) &&
				...
				msg = LocaliseMessage("The file '^0' has been modified outside Zxite. Should it be saved?",
					filePath.AsInternal());
