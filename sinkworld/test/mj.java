// SinkWorld demonstration program - Java version
...
class mj {
	...
	public static void assertValue(SplitText p, String s) {
		...
		for (int i=0; i<len; i++)
			SWTest.Assert((byte)s.charAt(i) == v[i]);
	}
	...
	public static void main(String[] args) {
		...
		IStyles astyles = SplitText.CreateStyles(1);
		abuffer.AttachStyles(astyles, null, 0);
		abuffer.InsertString(0, asp.Data(), asp.Length());
		Styler has = new Styler(abuffer, astyles, 0, abuffer.Length());
		la.Colourise(has);
		...
		bigbuffer.AttachStyles(bigstyles, lx, 0);
