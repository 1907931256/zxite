...
void add_ChangeLayer() {
    class_< ChangeLayer, boost::noncopyable, ChangeLayer_Wrapper >("ChangeLayer", init<  >())
        .def_readwrite("next", &ChangeLayer::next)
        .def_readwrite("position", &ChangeLayer::position)
        .def_readwrite("layer", &ChangeLayer::layer)
        .def("FindNextChange", pure_virtual(&ChangeLayer::FindNextChange))
    ;
